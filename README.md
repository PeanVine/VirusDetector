...
| 域名年龄评分 | **60** | 基于 Whois API 的 S 型衰减函数计分，新注册域名更可疑 |
...
│   ├── whois-client.js                # Whois API 查询客户端（内存缓存 + 速率限制）
...
#### 6. 域名年龄评分（Whois API | 最高 60 分）

通过 [WhoisCX 免费 API](https://whoiscx.com/apidoc/) 查询域名注册天数（`creation_days`），使用 S 型衰减函数计分：
